<template>
  <div id="nav">
    <!-- side nav -->
    <div class="lg:w-1/5 border-r border-red-300 px-2 lg:px-3 py-1 flex flex-col justify-between">
      <div>
        ...
        <div>
          <button class="focus:outline-none hover:text-white flex items-center py-2 px-4 hover:bg-red-600 rounded-full mr-auto mb-3 ">
          ...
          <!-- Not working button -->
          <button :key="tab" v-for="tab in tabs"  @click="id = tab.id" :class="`focus:outline-none hover:text-white flex items-center py-2 px-4 hover:bg-red-600 rounded-full mr-auto mb-3 ${ id === tab.id ? 'text-blue' : ''}`">
            <i :class="`${ tab.icon } text-2xl mr-4 text-left`"></i>
            <p class="text-lg font-semibold text-left hidden lg:block"> {{ tab.title }} </p>
          </button>
        </div>
        <button class="text-white bg-red-600 rounded-full font-semibold focus:outline-none w-12 h-12 lg:h-auto lg:w-full p-3 hover:bg-darkblue">
          <p class="hidden lg:block">Mes Postes</p>
          <i class="fas fa-plus lg:hidden"></i>
        </button>
      </div>
      <div class="lg:w-full relative">
        <button @click="dropdown = true" class="flex items-center w-full hover:bg-red-600 rounded-full p-2 focus:outline-none">
        ...
        <div v-if="dropdown === true" class="absolute bottom-0 left-0 w-64 rounded-lg shadow-md border-red-500 bg-red-500 mb-16">
