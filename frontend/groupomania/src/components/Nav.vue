<template>
    <div class="md:flex">
      <!--mobile menu bar -->
    <div class="grid grid-flow-col grid-cols-2 md:hidden">
      <router-link :to='`/`' >
        <div class="p-4 align-center flex justify-center">
          <img src="../assets/icon/icon.png" alt="logo" class="h-10 w-10 rounded-full flex-none"/>
        </div>
      </router-link>
    ...
    <div class="sidebar border-r border-lighter flex flex-col justify-between w-60 space-y-6 py-2 px-2 absolute inset-y-0 left-0 transform -translate-x-full md:relative md:translate-x-0 transition duration-200 ease-in-out bg-white md:bg-transparent">
      <div>
      ...
      <div class="lg:w-full  relative">
          <button @click="toggleDropdown() " class="flex hover:text-red-50 items-center w-full hover:bg-red-600 rounded-full p-2 focus:outline-none">
          ...
          <div :class="{'hidden': !dropdownPopoverShow, 'block': dropdownPopoverShow}" class="absolute text-red-50 bottom-0 left-0 w-64 rounded-lg shadow-md border-red-500 bg-red-500 mb-16">
            <router-link :to="`/profile`">
              <button @click="dropdown = false" class="p-3 flex items-center w-full hover:bg-red-400 p-2 focus:outline-none">
                ...
                <i class="fas fa-check ml-auto test-black"></i>
              </button>
            </router-link>
            <span @click="logout()">
